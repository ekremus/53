...
async function metrics(session) {
  return evaluate(session, `(() => {
    ...
    return {
      ...
      standingsFirstPlayer: document.querySelector('.stats-table tbody .stats-player span')?.textContent.trim() ?? null,
      standingsActiveSort: document.querySelector('.stats-sort.is-active')?.dataset.sortStandings ?? null,
      standingsAriaSort: document.querySelector('.stats-sort.is-active')?.closest('th')?.getAttribute('aria-sort') ?? null,
      standingsSortDirection: document.querySelector('.stats-sort.is-active .stats-sort__direction')?.textContent.trim() ?? null,
      standingsSortButtons: [...document.querySelectorAll('.stats-sort')].map((element) => ({
        key: element.dataset.sortStandings,
